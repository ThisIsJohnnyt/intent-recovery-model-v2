...
```
C:\Users\<you>\.venvs\intent-recovery-v2
```
...
`OneDrive\Desktop\intent-recovery-model-v2\training\venv\...` (the project's
old location before it moved to `C:\Claude Projects\intent-recovery-model-v2`),
the full path exceeded Windows' 260-character limit and `pip install` failed
with `OSError: [WinError 206] The filename or extension is too long`. Fixing
...
`OneDrive` sidestepped it entirely — and is arguably better practice anyway,
since a multi-GB ML environment inside a OneDrive-synced folder means
OneDrive tries to sync all of it. The project itself has since moved out of
`OneDrive\Desktop` too, but the venv is left at its dedicated short path
regardless — no reason to move it back under `training/venv/` now.
...
```powershell
python -m venv C:\Users\<you>\.venvs\intent-recovery-v2
C:\Users\<you>\.venvs\intent-recovery-v2\Scripts\python.exe -m pip install --upgrade pip
...
C:\Users\<you>\.venvs\intent-recovery-v2\Scripts\python.exe -m pip install torch --index-url https://download.pytorch.org/whl/cu132

# Everything else, from regular PyPI
C:\Users\<you>\.venvs\intent-recovery-v2\Scripts\python.exe -m pip install transformers sentencepiece accelerate numpy
```
...
```powershell
C:\Users\<you>\.venvs\intent-recovery-v2\Scripts\python.exe prepare_data.py
C:\Users\<you>\.venvs\intent-recovery-v2\Scripts\python.exe train.py
```

## Verifying CUDA is actually being used

```powershell
C:\Users\<you>\.venvs\intent-recovery-v2\Scripts\python.exe -c "import torch; print(torch.cuda.is_available(), torch.cuda.get_device_name(0))"
```
