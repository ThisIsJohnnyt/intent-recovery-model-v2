...
- **No dropped imperatives** (the only under-extraction rule in this
  ...
  **Imperative mood alone does not make something an action item** — settled
  by the product owner, 2026-09-07, after the thirteenth adversarial
  re-review found a reviewer (Gemini) applying this rule to REJECT four
  `zero_action_items` examples built entirely from bare imperatives ("watch
  movies, play with the dog") on the theory that any imperative must
  produce an `action_items` entry. Checked directly against
  `real_validation.jsonl`'s real note #15 — hand-written by the product
  owner, using this exact pattern with `action_items: []` — that reading
  would call his own real note wrong. **The actual test, in his words:**
  people genuinely meander between thoughts, and "watch movies, play with
  the dog" are self-suggested *possible* activities, not a distinct call to
  action — contrast "I'm going to watch SLC Punk and Hamilton tomorrow,"
  which names specific objects (two named films) with a direct action taken
  on them (being watched) and a real commitment behind it. The first is
  left blank on purpose because nothing was actually decided; the second is
  a genuine action item. This rule's worked example above ("don't forget
  the framework from that one article") is still a real commitment despite
  its unresolved reference — the distinguishing test is not whether the
  object is named, but whether the sentence commits to a specific,
  determined action at all, versus gesturing at open-ended possibilities.
  See `docs/datasets/CATEGORY_REFERENCE.md`'s "Thirteenth adversarial
  re-review" section for the full disagreement and reasoning.
...
**Process**: two independent passes, neither with involvement in writing
the sampled batches' generation prompts or taxonomy revisions, each given
only this file's items 0–8 and the sampled examples — **not** the original
review's conclusions or reasoning for accepting them, and not the other
pass's verdicts. Both instructed to look adversarially: assume nothing
already reviewed is necessarily correct, and report findings independent
of what the other pass concluded.

- **Gemini pass**: run as a fresh subagent the standing Antigravity
  session (`review_bridge/`) spawns for this specific re-review
  (`invoke_subagent`, per its own description a clean context window with
  no access to the ongoing bridge conversation) — not answered from that
  session's own accumulated context, and not the batch-generation
  `gemini-query`/API call, which is used only for generating new corpus
  examples and never for review. This split matters: the standing bridge
  session may itself have aligned on a sampled batch's design (an
  engineering-proposal-style bridge round), so answering from its own
  context would reintroduce the same correlation-bias risk this whole
  section exists to prevent, just relocated to Gemini's side. Send the
  checklist + sample via `review_bridge/ClaudeProposal.md`; the session
  relays its subagent's findings into `GeminiReview.md` without
  editorializing. See [PDR-006](../decisions/PDR-006.md)'s 2026-09-07
  amendment.
- **Claude pass**: a genuinely fresh subagent, spawned with no exposure to
  this session's reasoning, same as before.
...
| 2026-09-07 | 13 examples after batches 28-29, thirteenth periodic re-review — first run using the new Claude<->Gemini review bridge's plain `gemini-query` channel for the first pass (not the bridge's `review_bridge/` alignment loop, which is for engineering/pipeline proposals, not corpus content), then a genuinely fresh Claude subagent (no exposure to this session's reasoning) as the independent second pass, per `PDR-006`'s amendment. 7 `zero_action_items` examples (3 from batch 28's "pure reflection" set, 4 from batch 29's new imperative-syntax set, including the one record this session's own first-pass review had already fixed), 3 from batch 28's "near-miss contrast" set, 1 `contradictory_statement`, 2 never-touched free-mix controls | Gemini: 0 ACCEPT, 8 FIX, 5 REJECT (all 5 REJECTs on the batch-29 `zero_action_items` examples plus one on a `rapid_branching`-labeled example, all on the theory that a bare imperative in `input` must always produce an `action_items` entry). Fresh Claude: 4 ACCEPT, 9 FIX, 0 REJECT. Reconciled: 9 confirmed content fixes; all 5 of Gemini's REJECTs declined; 2 category-fit questions surfaced rather than resolved | **The most consequential disagreement of any re-review so far, because it goes to a category's own core definition rather than a wording nuance, and it directly implicates content this same session generated a few hours earlier.** Gemini REJECTed all four of batch 29's new `zero_action_items` examples (deliberately imperative-mood self-care/downtime notes — "watch movies, play with the dog", "walk to the park without checking the time") on the theory that `REVIEW_GUIDE.md` §4's "no dropped imperatives" rule makes any bare imperative in `input` an automatic action item. **This was checked directly against the project's own real, hand-written ground truth rather than argued abstractly**: `datasets/real_validation.jsonl`'s note #15 — the actual real note whose 2026-09-07 eval failure motivated batch 29 in the first place — uses this exact pattern ("Step away from the computer. ... Watch movies, play with the dog.") and its `action_items` is `[]`, hand-labeled by the product owner himself, never touched by any model. Gemini's rule, applied literally, would make that real, authoritative example wrong too. The fresh Claude pass, given no exposure to this reasoning or to Gemini's verdicts, independently reached ACCEPT on all four and articulated the actual distinguishing test unprompted: genuine action items carry an external obligation with a checkable completion state and a real cost to forgetting (call Maya, buy coffee beans, find the sketchbook in the attic); the batch-29 examples are self-permission for how to spend unstructured time, with no external party to disappoint and no coherent "I forgot to lay on the couch" failure state. Both passes' full checklist output on the OTHER 6 zero_action_items-adjacent examples in the sample (three earlier pure-reflection notes, all lacking any imperative at all) show the same tell: Gemini FIXed those for ordinary content nits, never REJECTed them — its objection is specifically and only triggered by imperative *mood*, independent of whether a real obligation exists. That is not a subtle reviewer preference; **it is the identical miscalibration the trained model itself showed on real note #15, and the exact failure batch 29 was generated to correct** — a striking, unplanned confirmation that the diagnosis was right, landing inside the review meant to catch it. **Declined all 5 REJECTs** (the 4 above, plus one on `#508`'s `rapid_branching`-labeled record for the same reason it applied to a genuine action item there — Gemini treated the correctly-non-empty action item as somehow still disqualifying, inconsistent with its own rule) as overreach, per the fresh pass's reasoning and the real-tier ground truth. **Escalated rather than resolved unilaterally, per this project's reconciliation rule, and settled directly by the product owner**: he concurred all 5 REJECTs don't hold, and gave the actual distinguishing test in his own words — "watch movies, play with the dog" are self-suggested possible activities people meander toward, not a distinct call to action, unlike "I'm going to watch SLC Punk and Hamilton tomorrow," which names specific objects with a real commitment behind them. Written into §4's "No dropped imperatives" bullet above as a durable rule. See `CATEGORY_REFERENCE.md`'s "Thirteenth adversarial re-review" section for the full write-up. **9 confirmed content fixes** (invented motive/timing/reaction-framing removed from 3 records, an unmarked ambiguity resolved back to the input's own vague word, a narrowed vague phrase restored to its original vagueness, two dropped hedges restored, an invented intensifier removed, one bullet's implied timing corrected, one narrative's invented means-end relationship un-nested) — full detail in `CATEGORY_REFERENCE.md`. **Two category-fit questions flagged rather than decided**: `#508`'s `rapid_branching` label doesn't cleanly match `TAXONOMY.md`'s own "several sub-ideas branching in quick succession" definition (a single linear reflective train of thought, previously relabeled once already from `topic_switching` during batch 28's own review — this may be the second wrong category in a row for the same record); `#514`'s `contradictory_statement` label doesn't clearly show a stated stance later reversed, reading closer to sequential option-weighing that resolves to indecision. Neither relabeled without the product owner's input, consistent with "documented rather than decided" precedent. **Several of Gemini's causal-connective complaints declined as the same overreach pattern the seventh re-review already documented** — flagging "because"/"as"/"so" connecting two facts the input already states adjacently in the same sentence or breath (e.g. "so frustrating, nobody even listens" — the input's own comma splice already reads as cause-and-effect) is not the invented-causality rule's target; that rule is for asserting a relationship between fragments that are actually unrelated. Copy ratios checked directly for the whole sample rather than argued impressionistically: max 0.833, none above this project's own calibrated 0.85 flag line, so several of Gemini's general "near-verbatim narrative" claims were declined as not meeting the project's own measured threshold for concern. Full three-check voice-regression suite, schema validation, duplicate check, and copy-ratio check all clean at 533/533 after the 9 fixes (same 12 pre-existing flagged duplicate pairs, none new). Corpus 533 → 533 (no rejections, no relabels — every fix in place at the time). **Addendum, same day**: the two category questions were then discussed with Gemini directly (a design discussion, not a blind re-review) and resolved by the product owner — `#514` relabeled `contradictory_statement` → `rapid_branching`; `#508` removed from the corpus outright rather than forced into a label, since it didn't cleanly fit any of the 15 categories. Corpus 533 → 532. Full detail in `CATEGORY_REFERENCE.md`'s "Two category-fit questions, resolved 2026-09-07" section. Next periodic re-review due after batch 31. |
| 2026-09-15 | 16 examples, fourteenth periodic re-review — triggered not by a new batch (none landed since the 13th; "after batch 31" never happened) but by `self_harm_context_boundary.jsonl`'s first retrain, per the standing rule that this specific event forces a re-review regardless of batch-count timing. What actually changed since the 13th re-review turned out to be scattered mid-file edits across 101 records (PDR-013's invented-emotion fixes, PDR-014's `action_items` ownership sweep, an external Fable/Claude-API review round, and the `#508`/`#514` addendum), not an appended batch — found by a precise `git diff --unified=0` against the last-reviewed commit rather than trusted from memory, since this session's own tracking of "what got touched" (~38 records) turned out to undercount by more than half. `generate_blind_rereview_prompt.py` extended with a `--touched-lines` flag (backward-compatible; `--since-line` still works for a normal appended-batch review) to select this exact scattered set as the "new" pool. Gemini first (`gemini-flash-lite-latest`, via the FARTS-relayed "zero-trust" audit format, not the standing review-bridge channel), then a genuinely fresh Claude subagent, per `PDR-006`'s amendment. 11 from the touched pool, 5 never-touched controls | Gemini: 5 PASSED, 11 FAILED (its own "zero-trust" scoring, not this project's ACCEPT/FIX/REJECT scale). Fresh Claude: 8 ACCEPT, 8 FIX, 0 REJECT. Reconciled: 6 confirmed fixes across 6 records (one record had two independent confirmed defects, one per reviewer) | **A stale checklist caused real, confirmed false positives on both passes, from two different angles, before this review even started.** `generate_blind_rereview_prompt.py`'s embedded `CHECKLIST_TEXT` had never been updated for PDR-014's `action_items` ownership reversal (2026-09-11) — a gap in PDR-014's own Scope section, which listed `REVIEW_GUIDE.md`/`DATASET_SPEC.md` but not this script. Fresh Claude, working from the stale rule, flagged 4 `multi_person_note` records (`#1`/line 214, `#10`/line 109, `#11`/line 145, `#12`/line 295 in this sample) for "missing" a named third party's commitment in `action_items` — exactly the pre-PDR-014 convention the checklist still stated. All 4 declined as false positives once checked against the actual, current written rule. Fixed directly in the script (not just declined this round) so future re-reviews don't repeat it. **Checking the same 4 records directly rather than only declining the flagged claim is what found a real, independent defect neither reviewer's own reasoning surfaced**: line 109 (`#10`) still had `"Maybe Mark should also take the recycling out"` sitting in `action_items` — a genuine, uncaught PDR-014 sweep gap (Mark's *other* task, vacuuming, had been correctly removed from the same record; this one slipped through). Fixed by removing it, leaving the writer's own two tasks. **Six real, independently-verified fixes total, several catching each other's misses.** `#4`/line 486 (`minimal_fragment`): narrative "I'm thinking about the green one" invents an interpretive frame the bare input "the green one" gives no basis for — fresh Claude's catch; Gemini scored this record 100%/PASSED. `#9`/line 426 (`simple_list`): `bullets` merges "Dish soap and sponges" into one item while `action_items` splits them into two, a field disagreement — fresh Claude's catch; Gemini scored this 100%/PASSED too. Fixing this one required a second pass: splitting `bullets` to match `action_items` blew the 7-bullet schema cap (`prepare_data.py`'s hard validator caught it immediately), so the two fields were reconciled the other direction instead — merging `action_items` back into "Buy dish soap and sponges" to match `bullets`. `#11`/line 145 (`multi_person_note`): `action_items` has `"Tell mike to grab some extra xlr cables"` (lowercase) while this same record's own `bullets` correctly capitalizes "Mike"/"XLR" — Gemini's catch, confirmed real by direct comparison. `#13`/line 30 (`topic_switching`) had two independent, unrelated real defects, one per reviewer: narrative invented "at the community garden" with zero textual basis (fresh Claude — matches this project's own canonical "sleeping bag ≠ camping trip" example almost exactly) and `action_items` duplicated the same leaking-hose task as two separate entries (Gemini). `#15`/line 4 (`dangling_reference`): narrative swapped input's specific "he" for the vaguer "Someone," and `bullets` dropped the "he said" attribution entirely — both reviewers independently flagged this same record from different angles, the strongest convergent signal of the round. **Several of Gemini's other claims declined as overreach, each checked directly rather than taken on the strength of its "zero-trust" framing**: `#1`'s "invented chronology" (narrative just joins two of the same person's separate statements with "and" — no order or causal link asserted); `#2`'s claim cited "No non-recovery" (narrative not reorganizing enough) but the actual complaint described the opposite (too much reframing) — a rule-citation mismatch, and the underlying content is an ordinary, correct `self_correction` narrative; `#5`'s "invented frame" claim about "props for the second act are still unsettled" — refuted directly by input's own opening clause, "We are trying to figure out the props for the second act," which states exactly that; `#7`, `#8`, and `#10`'s (and fresh Claude's own) register-consistency complaints about hedge-preserving action-item phrasing ("should probably," "I think I'll...") — declined per this project's own established precedent (10th re-review) that a genuine stated hedge stays un-flattened even at the cost of register uniformity; `#12`'s "invented certainty" claim about "My own piece is gathering the primary sources tonight" — judged too marginal a paraphrase to meet this project's bar, similar to previously-declined claims of this shape. **`self_harm_context_boundary.jsonl` deliberately excluded from this sample's pool** — hand-authored ground truth by the product owner, same treatment as `real_validation.jsonl`/`real_holdout.jsonl`, not the kind of AI-drift content this process screens for. The hard-gate's own auto-reject blind spot for that file (flagged in `PDR-015.md`'s Scope as a known, separate, pre-existing issue) was fixed in the same pass as the ownership-rule fix, since both are the same class of "embedded checklist never updated for a later PDR" gap. **A separate, pre-existing measurement-precision issue surfaced by `check_copy_ratio.py`, unrelated to this round's edits**: 6 already-allowlisted records (lines 407, 422, 457, 463, 466, 471) now measure at their stored ceiling's exact boundary (0.999 ceiling, 1.000 measured) — confirmed via a direct before/after comparison against the pre-session commit that this condition already existed prior to any of today's edits, not something introduced by the 6 fixes above. Flagged for the product owner's own attention rather than silently resolved. Full schema validation (532/532), duplicate check (12 pre-existing allowlisted pairs, none new), copy-ratio check (0 non-allowlisted breaches after allowlisting `#4`'s fix with a matching rationale), and full `pytest tests/` (31/31) all clean after the 6 fixes. Corpus 532 → 532 (no rejections, no relabels — every fix in place). Next periodic re-review due after the next batch, or the next retrain incorporating new hand-authored content, whichever comes first. |
