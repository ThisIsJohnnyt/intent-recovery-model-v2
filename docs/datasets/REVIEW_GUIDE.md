...
# across 18 records corpus-wide, invisible to this script for weeks.
python -c "
import json, re
rows = [json.loads(l) for l in open('datasets/synthetic.jsonl', encoding='utf-8') if l.strip()]
third = re.compile(r'^\s*(Needs to|Plans to|Intends to|Wants to|Feels|Suspects|Hopes|Thinks|Finds|Has to|Notes)\b')
passive = re.compile(r'^\s*(The\s+\w[\w\s]*?|All\s+\w[\w\s]*?|\w[\w\s]*?)\s+(must|needs?|has|have|is|are)\s+(to\s+)?be\s+\w+ed\b', re.I)
for i, r in enumerate(rows, 1):
